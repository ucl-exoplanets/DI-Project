...
class CNN():
    def __init__(self, datapath, batch_size=32, epochs=100,
                 ...
                 checkpoint_dir='checkpoint', CV_num=1, dense_unit=128, model_type='org', test_path='test',
                 valid_size=0.2, psf_path="CNN", SNR=10,decay = 0.0):
    ...
    def data_preprocess(self, data, SNR):
        ...
        for i in range(len(data)):
            new_img, Nx, Ny = self.SNR_injection(data[i].reshape(64, 64), self.psf_pl, SNR=SNR)
        ...
        return dataset.reshape(-1, 64, 64, 1), label
    ...
    def return_heatmap(self, model, org_img,normalise = True):
        ...
        pooled_grads_value, conv_layer_output_value = iterate([org_img.reshape(-1, 64, 64, 1)])
    ...
    def SNR_injection(self, data, tinyPSF, SNR=20, verbose=False, num_pixel=4):
        ...
        while True:
            ...
            Nx = np.random.randint(0, high=64)
            Ny = np.random.randint(0, high=64)
        ...
        if verbose:
            ...
            plt.imshow(pad_data[pad_length:pad_length + 64, pad_length:pad_length + 64])
        return pad_data[pad_length:pad_length + 64, pad_length:pad_length + 64], Nx, Ny
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_type', type=str, default='org')
    ...
    DLmodel = CNN(datapath=args.datapath, batch_size=args.batch_size, epochs=args.epochs, droprate=args.droprate,
                  ...
                  output_name=args.output_name, checkpoint_dir=args.checkpt, CV_num=args.cv_num,
